<table class="table align-middle table-row-dashed fs-6 gy-5">
    <thead>
        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
            ...
            <th class="min-w-125px">Google</th>
    ...
    <tbody class="text-gray-600 fw-semibold">
        ...
        <tr>
            ...
            <td>{{$item->position_id != 0 ? $item->position->name : ''}}</td>
            <td>
                @if(Cache::has('is_employee_online'. $item->id) == 0)
                    <div class="badge badge-light fw-bold mt-3">{{$item->last_seen ? $item->last_seen->diffForHumans() : 'Never Logged In'}}</div>
                @endif
            </td>
            <td>
                <div class="badge badge-light-{{$item->google_id ? 'success' : 'danger'}} fw-bold">{{$item->google_id ? 'Enabled' : 'Disabled'}}</div>
            </td>
            <td>
                <div class="badge badge-light-{{$item->paypal ? 'success' : 'danger'}} fw-bold">{{$item->paypal ? 'Enabled' : 'Disabled'}}</div>
            </td>
